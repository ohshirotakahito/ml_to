# -*- coding: utf-8 -*-
"""
Created on Wed May 26 15:01:02 2021
...
def bnal_RR(path_load):
   ...
   #print('pass')
    ...
    #S_Table dataの再構成 LAP Table
    LAR_data=tdms_file['LAP Table']['Relative Averaged Data'].raw_data
    LAX_data=tdms_file['LAP Table']['MS_IDX'].raw_data
    LAT_data=tdms_file['LAP Table']['Base Time length [ms]'].raw_data
    LAB_data=tdms_file['LAP Table']['Base Time length [ms]'].raw_data
    #ST_data=tdms_file['LSP Table']['S TL [ms]'].raw_data
    #SL_data=tdms_file['LSP Table']['S DL [s]'].raw_data
    
    #S_Table dataの整数/少数/文字データ化
    LAR_data= [int(s) for s in LAR_data]
    LAX_data= [str(s) for s in LAX_data]
    LAT_data= [int(s) for s in LAT_data]
    LAB_data= [int(s) for s in LAB_data]
